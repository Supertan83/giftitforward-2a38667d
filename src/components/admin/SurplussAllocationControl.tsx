...
import {
  ArrowLeft, RefreshCw, Send, Package, Undo2, Loader2,
  Search, Plus, Trash2, ChevronDown, ChevronRight, ExternalLink,
  RotateCcw, List, History
} from 'lucide-react';
...
export const SurplussAllocationControl = ({ onBack }: SurplussAllocationControlProps) => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      toast({ title: 'Events Loaded', description: `Found ${Array.isArray(items) ? items.length : 0} marketplace events` });
    } catch (err) {
      toast({ title: 'Fetch Failed', description: err instanceof Error ? err.message : 'Error', variant: 'destructive' });
    } finally {
  ...
  const fetchEventAllocations = async (eventId: number) => {
    ...
    try {
      ...
      const items = result.data?.items || result.data?.data || result.data || [];
      setEventAllocations(Array.isArray(items) ? items : []);
    } catch (err) {
      toast({ title: 'Fetch Failed', description: err instanceof Error ? err.message : 'Error', variant: 'destructive' });
    } finally {
  ...
  const searchDonations = async () => {
    ...
    try {
      ...
      toast({ title: 'Search Failed', description: err instanceof Error ? err.message : 'Error', variant: 'destructive' });
    } finally {
  ...
  const executeBatchAllocate = async () => {
    ...
    try {
      ...
      toast({ title: 'Allocation Successful', description: `Allocated ${batchItems.length} items to event #${selectedEventId}` });
      setBatchItems([]);
      fetchEventAllocations(selectedEventId);
    } catch (err) {
      toast({ title: 'Allocation Failed', description: err instanceof Error ? err.message : 'Error', variant: 'destructive' });
    } finally {
  ...
  const executeUpdate = async (allocationId: number) => {
    ...
    try {
      const result = await callApi({ action: 'update_allocation', allocation_id: allocationId, amount });
      if (!result.success) throw new Error(result.error || 'Update failed');
      toast({ title: 'Allocation Updated', description: `Allocation #${allocationId} updated to ${amount}` });
      setUpdateAmount('');
  ...
  const executeReturnRemaining = async (allocationId: number) => {
    setReturningId(allocationId);
    try {
      const result = await callApi({ action: 'return_remaining', allocation_id: allocationId });
      if (!result.success) throw new Error(result.error || 'Return failed');
      toast({ title: 'Items Returned', description: `Remaining items from allocation #${allocationId} returned to donation pool` });
      if (selectedEventId) fetchEventAllocations(selectedEventId);
    } catch (err) {
      toast({ title: 'Return Failed', description: err instanceof Error ? err.message : 'Error', variant: 'destructive' });
    } finally {
      setReturningId(null);
  ...
  // Delete allocation
  const executeDelete = async (allocationId: number) => {
    try {
      const result = await callApi({ action: 'delete_allocation', allocation_id: allocationId });
      if (!result.success) throw new Error(result.error || 'Delete failed');
      toast({ title: 'Allocation Deleted', description: `Allocation #${allocationId} removed` });
      if (selectedEventId) fetchEventAllocations(selectedEventId);
    } catch (err) {
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">Create, update, and return allocations directly on the Surpluss platform</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
        <TabsList className="mb-4">
          <TabsTrigger value="allocate" className="gap-1.5"><Plus className="h-3.5 w-3.5" />Allocate</TabsTrigger>
          <TabsTrigger value="manage" className="gap-1.5"><List className="h-3.5 w-3.5" />Manage</TabsTrigger>
            ...
            <CardContent>
              <div className="flex gap-2 mb-3">
                <Button onClick={fetchEvents} disabled={isLoadingEvents} size="sm">
                  {isLoadingEvents ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" /> : <RefreshCw className="h-4 w-4 mr-1.5" />}
                  Load Events
                </Button>
                {selectedEvent && (
                    ...
                    <TableBody>
                      {events.map(event => (
                        <TableRow key={event.id} className={selectedEventId === event.id ? 'bg-primary/5' : ''}>
                          ...
                          <TableCell>
                            <Badge variant={event.status === 'PLANNED' ? 'outline' : event.status === 'COMPLETED' ? 'secondary' : 'default'} className="text-xs">
                              {event.status || '—'}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            <Button
                              ...
                              onClick={() => { setSelectedEventId(event.id); fetchEventAllocations(event.id); }}
              ...
              <CardContent>
                <div className="flex gap-2 mb-3">
                  ...
                  <Button onClick={searchDonations} disabled={isSearchingDonations} size="sm">
                    {isSearchingDonations ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
                  </Button>
                </div>
                {donations.length > 0 && (
                        ...
                        {donations.map(d => {
                          ...
                          return (
                            <TableRow key={d.id}>
                              ...
                              <TableCell className="text-xs text-muted-foreground">{d.material_group?.name || '—'}</TableCell>
                ...
                {batchItems.length > 0 && (
                  <div className="border rounded-md p-3 bg-muted/30">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => setBatchItems([])}>Clear All</Button>
                    </div>
                    <div className="space-y-1.5 mb-3">
                      {batchItems.map(b => (
                        <div key={b.material_id} className="flex items-center justify-between text-sm bg-background rounded px-2 py-1">
                          <span className="truncate flex-1">#{b.material_id} — {b.material_title}</span>
                          <span className="font-mono text-xs mx-2">×{b.amount}</span>
                          <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => removeBatchItem(b.material_id)}>
                    ...
                    <Button onClick={executeBatchAllocate} disabled={isAllocating} className="w-full">
                      {isAllocating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Send className="h-4 w-4 mr-2" />}
        ...
        <TabsContent value="manage" className="space-y-4">
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>View, update amounts, and return remaining items for a marketplace event</CardDescription>
            </CardHeader>
            <CardContent>
              {/* Event selector */}
              <div className="flex gap-2 mb-4">
                ...
                {events.length > 0 && (
                  <Select value={selectedEventId?.toString() || ''} onValueChange={(v) => { const id = parseInt(v); setSelectedEventId(id); fetchEventAllocations(id); }}>
                      ...
                      {eventAllocations.map(alloc => (
                        <TableRow key={alloc.id}>
                          ...
                          <TableCell className="text-sm">
                            {alloc.donation_metadata?.title || `Material #${alloc.material_id}`}
                            {alloc.donation_metadata?.material_group?.name && (
                              <span className="text-xs text-muted-foreground ml-1">({alloc.donation_metadata.material_group.name})</span>
                            )}
                          </TableCell>
                          <TableCell className="text-right font-mono">{alloc.amount ?? '—'}</TableCell>
                          <TableCell className="text-right font-mono text-emerald-600">{alloc.distributed_amount ?? '—'}</TableCell>
                          <TableCell className="text-right font-mono">{alloc.remaining_amount ?? '—'}</TableCell>
                          <TableCell>
                            <div className="flex gap-1">
                              ...
                              <div className="flex gap-1 items-center">
                                <Input
                                  ...
                                  value={updatingId === alloc.id ? updateAmount : ''}
                                  onChange={(e) => { setUpdatingId(alloc.id); setUpdateAmount(e.target.value); }}
                                />
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className="h-7 text-xs"
                                  disabled={updatingId === alloc.id && !updateAmount}
                                  onClick={() => executeUpdate(alloc.id)}
                                >
                                  {updatingId === alloc.id ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Set'}
                                </Button>
                              </div>
                              {/* Return remaining */}
                              <Button
                                ...
                                onClick={() => setConfirmReturnId(alloc.id)}
                              >
                                {returningId === alloc.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <RotateCcw className="h-3 w-3" />}
                              </Button>
                              {/* Delete */}
                              <Button
                                size="sm"
                                ...
                                onClick={() => executeDelete(alloc.id)}
                              >
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={fetchAuditLog} disabled={isLoadingAudit} size="sm" variant="outline">
                  {isLoadingAudit ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" /> : <RefreshCw className="h-4 w-4 mr-1.5" />}
                  Refresh
                </Button>
              </div>
            </CardHeader>
      ...
      <AlertDialog open={confirmReturnId !== null} onOpenChange={() => setConfirmReturnId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Return Remaining Items?</AlertDialogTitle>
            <AlertDialogDescription>
              This will return all undistributed items from allocation #{confirmReturnId} back to the Surpluss donation pool. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => confirmReturnId && executeReturnRemaining(confirmReturnId)}>
              Return Items
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
